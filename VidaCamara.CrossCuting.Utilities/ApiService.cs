...
namespace VidaCamara.CrossCuting.Utilities
{
    public class ApiService<T, U> where T : class
    {
        ...
        public static T PostRequest(string url, U postObject, string token)
        {
            T result = null;
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    client.Timeout = TimeSpan.FromSeconds(int.Parse(UtilHelper.obtainConfig("tiempoRespuestaServiciosIC")));
                    client.DefaultRequestHeaders.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            ...
            {
                Log.save(null, "ERROR EN SERVICIO INFOCORE" + ex.Message);
            }
